...
def test_shared_result_chart_does_not_shutdown_mt5(tmp_path,base_config):
    fake=FakeMT5Gateway()
    fake.rates=[{'time':1_700_000_000+i*300,'open':100+i*.01,'high':101+i*.01,'low':99+i*.01,'close':100.5+i*.01} for i in range(30)]
    signal={'signal_id':'NX-001','symbol':'XAUUSD','mt5_symbol':'XAUUSD','direction':'BUY','timeframe':'M5','entry':100,'tp':102,'sl':99}
    ...
    assert result['ok'] and fake.shutdown_calls==0
